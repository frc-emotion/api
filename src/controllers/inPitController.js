...
const editProfile = asyncHandler(async (req, res) => {
	try {
		...
		if (!exists) {
			...
			const modifiedDB = await inPit.findOne({
				teamNumber: req.body.teamNumber,
			});
				...
				for ([dbKey, dbValue] of entrifiedDB) {
					if (key === dbKey && dbValue instanceof Array) {
						...
						if (!all[0].includes(value.content)) {
							const updated = await inPit.findOneAndUpdate(
								{ teamNumber: req.body.teamNumber },
								{ $push: { [key]: value } },
								{ new: true }
							);
			...
			const profile = await inPit.findOne({
				teamNumber: req.body.teamNumber,
			});
			res.status(200).json(profile);
		}
...
const deleteProfile = asyncHandler(async (req, res) => {
	try {
		...
		!profile ? res.status(404).end() : await profile.remove();
		res.status(200).end();
	} catch (e) {
		res.status(500).json({ message: "Error" });
		return;
